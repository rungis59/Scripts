...
SET @customoper = 'ITS SQL DBA'
SET @jobowner = 'sa'
SET @JobName = N'_Maintenance_'+(REPLACE((SELECT @@servername),'\','_'))+'_Daily Cycle Errorlog'
...
BEGIN
	EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=@JobName, 
		...
		@notify_level_page=2, 
		@delete_level=0, 
		@description=N'Cycles Errorlog when its size is over 200MB or its age over 15 days.', 
...
END 
+ CASE WHEN LogFileSizeBytes > 209715200 THEN '' The ErrorLog will be cycled because of its size.'' -- over 200MB
